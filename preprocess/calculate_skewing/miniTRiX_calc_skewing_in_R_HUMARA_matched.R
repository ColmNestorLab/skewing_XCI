...
dataz <- fread("data/data_rerun_by_Bjorn/miniTRiX_HUMARA_comparison_data_blue_channel.csv")
...
# Fix name of the 317727 sample and remove the initial erroneous run.
final_table_wide_fixed <- final_table_wide_fixed[!final_table_wide_fixed$sample %in% "317727",]
final_table_wide_fixed[final_table_wide_fixed$sample %in% "317727om",]$sample <- "317727"

# Export
write.table(final_table_wide_fixed, "data/minitrix_HUMARA_matched_results.tsv", sep = "\t", quote = F, row.names = F)
