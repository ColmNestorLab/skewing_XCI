...
dataz[which(dataz$Size > 275 & dataz$Size < 350),]$gene <- "CNKSR2"
dataz[which(dataz$Size > 350 & dataz$Size < 430),]$gene <- "TCAC1"
...
hue <- unique(data_filt_with_meta[,c("date_run", "sample")]) %>% dplyr::group_by(sample) %>% dplyr::count()

# Find the two highest values per group with a separation of at least 2.5
two_highest_peaks_per_undigested_sample <- data_filt_with_meta %>%
  filter(digest == "U") %>%
...
digested_samples <- data_filt_with_meta[data_filt_with_meta$digest == "D",]
...
#stored_below <- df[df$Height < 2000 & df$gene == "CNKSR2" & df$digest == "U",]
...
# calculate skewing (from PMID: 36412098)
...
df_wide_calc <- 
  ...
  dplyr::summarise(
    ...
    CNKSR2_ratio_mock = sum(as.numeric(U_CNKSR2_HIGH) / as.numeric(U_CNKSR2_LOW), na.rm = T),
    CNKSR2_ratio_digest = sum(as.numeric(D_CNKSR2_HIGH) / as.numeric(D_CNKSR2_LOW), na.rm = T),
    ...
    TCAC1_ratio_mock = sum(as.numeric(U_TCAC1_HIGH) / as.numeric(U_TCAC1_LOW), na.rm = T),
    TCAC1_ratio_digest = sum(as.numeric(D_TCAC1_HIGH) / as.numeric(D_TCAC1_LOW), na.rm = T),
    
    ZIC3_ratio_mock = sum(as.numeric(U_ZIC3_HIGH) / as.numeric(U_ZIC3_LOW), na.rm = T),
    ZIC3_ratio_digest = sum(as.numeric(D_ZIC3_HIGH) / as.numeric(D_ZIC3_LOW), na.rm = T)
  )

# get column names in which I want to change anything above 2.5 to NA (and thus excluding homozygous repeats).
ratio_mock_column_names <- c("ARHGAP6_ratio_mock", "CNKSR2_ratio_mock", "RP2_ratio_mock", "TCAC1_ratio_mock", "ZIC3_ratio_mock")
...
df_wide_calc_final <- 
  ...
  dplyr::summarise(
    ...
    CNKSR2_ratio_normalized = sum(CNKSR2_ratio_digest/CNKSR2_ratio_mock),
    CNKSR2_skewing =  round(abs(CNKSR2_ratio_normalized/(CNKSR2_ratio_normalized + 1)) * 100,1),
    CNKSR2_skewing =  ifelse(CNKSR2_skewing > 50, yes = CNKSR2_skewing, no = 100-CNKSR2_skewing),
    ...
    TCAC1_ratio_normalized = sum(TCAC1_ratio_digest/TCAC1_ratio_mock),
    TCAC1_skewing =  round(abs(TCAC1_ratio_normalized/(TCAC1_ratio_normalized + 1)) * 100,1),
    TCAC1_skewing =  ifelse(TCAC1_skewing > 50, yes = TCAC1_skewing, no = 100-TCAC1_skewing),
    
    ZIC3_ratio_normalized = sum(ZIC3_ratio_digest/ZIC3_ratio_mock),
    ZIC3_skewing =  round(abs(ZIC3_ratio_normalized/(ZIC3_ratio_normalized + 1)) * 100,1),
    ZIC3_skewing =  ifelse(ZIC3_skewing > 50, yes = ZIC3_skewing, no = 100-ZIC3_skewing),
    
    mean_skewing = round(mean(na.rm = T, c(ARHGAP6_skewing, CNKSR2_skewing, RP2_skewing, TCAC1_skewing, ZIC3_skewing)),1)
) %>% dplyr::select(-ARHGAP6_ratio_normalized, -CNKSR2_ratio_normalized, -RP2_ratio_normalized, -TCAC1_ratio_normalized, -ZIC3_ratio_normalized)
...
final_table_wide_fixed <- dplyr::select(final_table_wide_fixed, "sample", "mean_skewing",
                                        ...
                                        "CNKSR2_skewing","CNKSR2_allele1","CNKSR2_allele2",
                                        "TCAC1_skewing","TCAC1_allele1","TCAC1_allele2",
                                        "RP2_skewing","RP2_allele1","RP2_allele2",
                                        "ZIC3_skewing","ZIC3_allele1","ZIC3_allele2")

# Remove the skewing and allele values based on failed runs.
final_table_wide_fixed[final_table_wide_fixed$sample %in% failed_CNKSR2,]$CNKSR2_skewing <- NA
final_table_wide_fixed[final_table_wide_fixed$sample %in% failed_ARHGAP6,]$ARHGAP6_skewing <- NA

# make numeric
final_table_wide_fixed$ARHGAP6_skewing <- as.numeric(final_table_wide_fixed$ARHGAP6_skewing)
final_table_wide_fixed$CNKSR2_skewing <- as.numeric(final_table_wide_fixed$CNKSR2_skewing)
final_table_wide_fixed$TCAC1_skewing <- as.numeric(final_table_wide_fixed$TCAC1_skewing)
final_table_wide_fixed$RP2_skewing <- as.numeric(final_table_wide_fixed$RP2_skewing)
final_table_wide_fixed$ZIC3_skewing <- as.numeric(final_table_wide_fixed$ZIC3_skewing)

# also make sure the mean is taken from the values that we can trust; i.e. not the one repeat that failed.
final_table_wide_fixed[final_table_wide_fixed$sample %in% failed_CNKSR2,]$mean_skewing <- final_table_wide_fixed[final_table_wide_fixed$sample %in% failed_CNKSR2,] %>% dplyr::summarise(mean_skewing = rowMeans(across(c(ARHGAP6_skewing, TCAC1_skewing, RP2_skewing, ZIC3_skewing)), na.rm = TRUE))
                                                                                               
final_table_wide_fixed[final_table_wide_fixed$sample %in% failed_ARHGAP6,]$mean_skewing <- final_table_wide_fixed[final_table_wide_fixed$sample %in% failed_ARHGAP6,] %>% dplyr::summarise(mean_skewing = rowMeans(across(c(CNKSR2_skewing, TCAC1_skewing, RP2_skewing, ZIC3_skewing)), na.rm = TRUE))


# Export
write.table(final_table_wide_fixed, "data/trix_results.tsv", sep = "\t", quote = F, row.names = F)
