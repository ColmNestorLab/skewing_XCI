...
dataz <- fread("data/data_rerun_by_Bjorn/miniTRiX_AGE_data_blue_channel.csv")
...
metadataz <- unique(fread("data/data_rerun_by_Bjorn/miniTRiX_AGE_metadata.csv", header = T)[,c("filename", "date_run")])
...
# split the table into first and second time point.
# First time point start with '2' or '3', second time point always with '8'.
first_timepoint <- final_table_wide_fixed %>%
  filter(grepl("^3|^2", as.character(sample)))
second_timepoint <- final_table_wide_fixed %>%
  filter(grepl("^8", as.character(sample)))

# Remove the first two characters of the sample name.
first_timepoint$sample_ID <- str_sub(as.character(first_timepoint$sample), 3)
second_timepoint$sample_ID <- str_sub(as.character(second_timepoint$sample), 3)

# merge
both_timepoints <- merge(first_timepoint, second_timepoint, by = c("sample_ID"), suffixes = c("_first","_second"), all = T)

# double-check same repeat lengths are detected.
both_timepoints$ARHGAP6_MATCH <- ifelse(both_timepoints$ARHGAP6_allele1_first == both_timepoints$ARHGAP6_allele1_second & both_timepoints$ARHGAP6_allele2_first == both_timepoints$ARHGAP6_allele2_second, yes = "MATCH", no = "FAIL")

both_timepoints$RP2_MATCH <- ifelse(both_timepoints$RP2_allele1_first == both_timepoints$RP2_allele1_second & both_timepoints$RP2_allele2_first == both_timepoints$RP2_allele2_second, yes = "MATCH", no = "FAIL")

both_timepoints$ARHGAP6_diff <- as.numeric(both_timepoints$ARHGAP6_skewing_first) - as.numeric(both_timepoints$ARHGAP6_skewing_second)
both_timepoints$RP2_diff <- as.numeric(both_timepoints$RP2_skewing_first) - as.numeric(both_timepoints$RP2_skewing_second)

mean(both_timepoints$ARHGAP6_diff, na.rm = T)
mean(both_timepoints$RP2_diff, na.rm = T)

# Export
write.table(both_timepoints, "data/minitrix_AGE_results.tsv", sep = "\t", quote = F, row.names = F)
