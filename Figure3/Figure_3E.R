...
distribution_skewing_across_tissues_top <- 
  ggplot(extremely_skewed_tissue_samples_count_dcast,
...
test_ridge <- 
  ggplot(plot_metrics_ridge, aes(x = median, y = tissue_id, fill = tissue_id)) +
  geom_density_ridges() +
  theme_AL_box_rotX() +
  theme(legend.position = "none")+
  geom_vline(xintercept = c(stats_stats_plot_metrics_order$median+2*stats_stats_plot_metrics_order$sd), lty=2)+
...
# summarize no. of skewed, not skewed and total tissue count.
summarize_extremely_skewed_tissue_samples_count_dcast <- extremely_skewed_tissue_samples_count_dcast %>%
  summarise(
    not_skewed = sum(not_skewed, na.rm = TRUE),
    skewed = sum(skewed, na.rm = TRUE),
    total = sum(total, na.rm = TRUE)
  )

# Summarize by age
df <- merge(unique(plot_metrics[,c("participant", "skewed")]), unique(GTEx_femmes_filt[,c("participant", "age")]),  by = c("participant"))

# Compute cumulative proportion of skewed samples by age
cum_df <- df %>%
  arrange(age) %>%
  mutate(is_skewed = ifelse(skewed == "skewed", 1, 0)) %>%
  mutate(cum_skewed = cumsum(is_skewed),
         cum_prop = cum_skewed / n())

# Find the age at which 12% of samples are skewed
threshold <- cum_df %>%
  filter(cum_prop >= 0.12) %>%
  dplyr::slice(1)

threshold_age <- threshold$age

ggplot(cum_df, aes(x = age, y = cum_prop)) +
  geom_line(color = "steelblue", size = 1.2) +
  geom_hline(yintercept = 0.12, linetype = "dashed", color = "darkred") +
  geom_vline(xintercept = threshold_age, linetype = "dotted", color = "darkred") +
  annotate("text", x = threshold_age, y = 0.13,
           label = paste0("12% skewed at age ≈ ", threshold_age),
           hjust = -0.1, color = "darkred", size = 3.5) +
  scale_y_continuous(labels = scales::percent_format(accuracy = 1)) +
  labs(
    x = "Age (years)",
    y = "Cumulative proportion skewed",
    title = "Cumulative proportion of skewed individuals by age"
  ) +
  theme_minimal(base_size = 13)

ggplot(plot_metrics_age, aes(x=age, col = skewed)) + geom_histogram(binwidth = 1)
