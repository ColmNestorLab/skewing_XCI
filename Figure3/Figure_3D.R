...
#suppl_table_2_metrics_order <- suppl_table_2_metrics_order[order(suppl_table_2_metrics_order$median, decreasing = F),]$participant

suppl_table_2_metrics_order2 <- suppl_table_2_metrics %>% dplyr::group_by(tissue_id) %>% rstatix::get_summary_stats(median, type = "common")
#suppl_table_2_metrics_order2 <- suppl_table_2_metrics_order2[order(suppl_table_2_metrics_order2$median, decreasing = F),]$tissue_id
...
suppl_table_2_metrics$skewed <- ifelse(suppl_table_2_metrics$median > (stats_stats_suppl_table_2_metrics_order$median+2*stats_stats_suppl_table_2_metrics_order$sd), yes = "skewed", no = "not_skewed")


#### Figure 3D, showing no. of skewed tissues per individual ####
countz <- suppl_table_2_metrics %>% dplyr::group_by(participant, skewed) %>% dplyr::count()
...
# Export source data
source_data_fig_3D <- large_df_frac_count
write.table(source_data_fig_3D, "source_data/Fig_3D_source_data.tsv", quote = F, row.names = F, sep = "\t")

#### Read in source data. ####
# Read in.
source_data_fig_3D <- fread("source_data/Fig_3D_source_data.tsv")

source("plot_parameters_TRiX.R")

frequencies_per_skewing_count <-
  ggplot(source_data_fig_3D, 
         aes(x=skewed_tissue_count, y=count)) + geom_bar(stat="identity") +
  geom_text(aes(label=count)) + 
  annotate(geom="text", 
           x=source_data_fig_3D$skewed_tissue_count, 
           y=-4.5, 
           label = paste0("1/", source_data_fig_3D$frac2))+
  theme_AL_box()+
  labs(x="skewed tissue (count)", y="GTEx females (count)")

ggsave2(filename = "01_plots/Figure_3D.pdf", width = 8, height = 4,
