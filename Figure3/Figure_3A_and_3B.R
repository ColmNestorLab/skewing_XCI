...
suppl_table_1_from_eLife_paper <- fread("anno/Supplementary_Table1.tsv")
...
# read in participant annotations. Metadata from GTEx.
...
# Export source data
source_data_fig_3b <- stats_suppl_table_2_metrics_order
...
stats_stats_suppl_table_2_metrics_order <- source_data_fig_3b %>% rstatix::get_summary_stats(median, type = "common")

#### Figure 3b, showing distribution of skewing in GTEx ####
median_nonPAR_ASE <- 
  ggplot(source_data_fig_3b, 
         aes(x=1, y=median)) +
  ...
  geom_text_repel(data=source_data_fig_3b[source_data_fig_3b$median > (stats_stats_suppl_table_2_metrics_order$median+2*stats_stats_suppl_table_2_metrics_order$sd) & source_data_fig_3b$participant %in% c("GTEX-UPIC", "GTEX-13PLJ", "GTEX-ZZPU"),], 
                  aes(label=participant), min.segment.length = 0.001, size = 3, box.padding = 0.1, nudge_x = 0.25)#, nudge_y = -0.05)



# Export source data
source_data_fig_3a <- stats_TRiXi_data
write.table(source_data_fig_3a, "source_data/Fig_3a_source_data.tsv", quote = F, row.names = F, sep = "\t")
...
# calculate descriptive statistics of descriptive statistics, to plot lines mean, and 1sd and 2sd.
stats_stats_TRiXi_data <- source_data_fig_3a %>% rstatix::get_summary_stats(mean, type = "common")

# Classify XCI pattern of individuals.
source_data_fig_3a$'XCI pattern' <- "mosaic"
source_data_fig_3a[source_data_fig_3a$mean > (stats_stats_TRiXi_data$mean+1*stats_stats_TRiXi_data$sd),]$'XCI pattern' <- "skewed"
source_data_fig_3a[source_data_fig_3a$mean > (stats_stats_TRiXi_data$mean+2*stats_stats_TRiXi_data$sd),]$'XCI pattern' <- "extremely skewed"
source_data_fig_3a$'XCI pattern' <- factor(source_data_fig_3a$'XCI pattern', levels = c("extremely skewed", "skewed", "mosaic"))
...
distribution_of_skewing_across_individual <- 
  ...
  geom_text_repel(data=source_data_fig_3a[source_data_fig_3a$sample %in% c("320280"),], 
